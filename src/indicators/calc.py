"""
技术指标计算统一模块（EMA / RSI / ATR / MACD）。

项目内所有指标计算的唯一来源，其他模块应 import 本文件，禁止重复实现。

用途：
1) 在 eval 数据模式 prompt 中补充指标上下文；
2) 供 pipeline / scripts 复用。
"""
...
def normalize_ohlcv_df(df: pd.DataFrame) -> pd.DataFrame:
    """统一 OHLCV DataFrame：time→timestamp 重命名、类型转换、去空、按时间排序。"""
    out = df.copy()
    ...
    if missing:
        raise ValueError(f"输入 DataFrame 缺少字段: {sorted(missing)}")
...
def add_all_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """添加 MA20/MA60 + MACD + RSI14 + ATR14（完整指标集）。"""
    out = add_macd_rsi(df)
    if out.empty:
        return out
...
def maybe_float(v: Any, ndigits: int = 4) -> float | None:
    if v is None:
        return None
    try:
        x = float(v)
...
def indicator_snapshot_from_rows(rows: list[dict[str, Any]], tail: int = 5) -> dict[str, Any]:
    ...
    last = out.iloc[-1]
    rsi_val = maybe_float(last.get("rsi14"), 2)
    if rsi_val is None:
        rsi_state = "na"
    elif rsi_val >= 70:
    ...
    macd_val = maybe_float(last.get("macd"), 4)
    signal_val = maybe_float(last.get("signal"), 4)
    hist_val = maybe_float(last.get("hist"), 4)
    if macd_val is None:
        macd_side = "na"
    else:
    ...
    for c in ("close", "rsi14", "macd", "signal", "hist"):
        if c in tail_rows.columns:
            tail_rows[c] = tail_rows[c].map(lambda x: maybe_float(x, 4))
